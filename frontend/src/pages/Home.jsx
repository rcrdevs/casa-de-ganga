...
export function Home() {
  ...
  const handleCheckout = async (couponCode = null) => {
    if (!token) {
      showToast('⚠️ Faça login para finalizar!', 'error');
      setShowLogin(true);
      return;
    }

    if (!shippingData.cep || !shippingData.endereco || !shippingData.numero) {
      showToast('⚠️ Preencha os dados de entrega!', 'error');
      return;
    }
    ...
    <div className="min-h-screen relative overflow-hidden font-['Space Grotesk'] bg-ganga-light text-ganga-black">
      ...
      {toast && (
        <div
          className={`fixed top-6 right-6 z-[500] px-5 py-3 rounded-lg shadow-xl border ${
            toast.type === 'error' 
              ? 'bg-ganga-red border-ganga-red-light text-white' 
              : 'bg-ganga-black border-ganga-gray text-white'
          }`}
        >
          {toast.text}
        </div>
        ...
        <div className="max-w-6xl mx-auto px-4 text-center">
          ...
          <div className="flex justify-center space-x-8 mb-8 text-3xl">
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="text-gray-500 hover:text-ganga-red-light transition-colors"
            >
              <FaInstagram />
            </a>
            <a
        ...
        {showReturnModal && (
          ...
          >
            <motion.div
              ...
              animate={{ scale: 1, y: 0 }}
              exit={{ scale: 0.9, y: 20 }}
              className="bg-ganga-black p-8 max-w-lg w-full relative rounded-xl shadow-2xl border border-ganga-gray"
            ...
            >
              ...
              <form action="https://formsubmit.co/contato@casadeganga.com" method="POST" className="mt-6 space-y-4">
                ...
                <textarea
                  ...
                  className="w-full bg-black border border-ganga-gray rounded-lg p-3 text-sm text-white h-32 focus:outline-none focus:border-ganga-red"
                  required
                ></textarea>
                <button
                  type="submit"
      ...
      <AnimatePresence>
        {selectedProduct && (
          ...
          >
            <motion.div
              className="bg-ganga-black rounded-xl max-w-4xl w-full relative border border-ganga-gray shadow-2xl"
              initial={{ scale: 0.9 }}
              animate={{ scale: 1 }}
              exit={{ scale: 0.9 }}
              onClick={(e) => e.stopPropagation()}
